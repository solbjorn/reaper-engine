...
/// Taken from https://iquilezles.org/articles/smoothsteps
float SLB_QuadraticRational1D(float x) { return x * x / (2.0 * x * x - 2.0 * x + 1.0); }
...
float2 SLB_VogelDisk_Sample(const int sample_index, const int samples_count, float noise)
{
    const float r = sqrt(float(sample_index) + noise) / sqrt(float(samples_count));
...
{
    const uint m = 2147483647u;
    const uint a1 = 3242174893u;
    const uint a2 = 2447445397u;
    return m + uint2(a1 * n, a2 * n);
}
...
uint SLB_HilbertIndex(uint2 uv)
{
    // Hilbert curve:
...
uint SLB_HilbertNoise(uint2 uv) { return SLB_R1(SLB_HilbertIndex(uv)); }

float SLB_HilbertNoise_float(uint2 uv) { return SLB_ToFloat1(SLB_HilbertNoise(uv)); }

uint2 SLB_HilbertNoise2D(uint2 uv) { return SLB_R2(SLB_HilbertIndex(uv)); }

float2 SLB_HilbertNoise2D_float(uint2 uv) { return SLB_ToFloat2(SLB_HilbertNoise2D(uv)); }
