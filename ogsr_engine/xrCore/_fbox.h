...
public:
    ...
    constexpr const T* data() const { return &min.x; }
    ...
    constexpr SelfRef set(T x1, T y1, T z1, T x2, T y2, T z2)
    {
        min.set(x1, y1, z1);
        max.set(x2, y2, z2);
        return *this;
    ...
    constexpr SelfRef setb(const Tvector& center, const Tvector& dim)
    {
        min.sub(center, dim);
        max.add(center, dim);
        return *this;
    ...
    constexpr SelfRef sub(const Tvector& p)
    {
        min.sub(p);
        max.sub(p);
        return *this;
    ...
    constexpr BOOL contains(T x, T y, T z) const { return (x >= x1) && (x <= x2) && (y >= y1) && (y <= y2) && (z >= z1) && (z <= z2); }
    constexpr BOOL contains(const Tvector& p) const { return contains(p.x, p.y, p.z); }
    constexpr BOOL contains(SelfCRef b) const { return contains(b.min) && contains(b.max); }

    constexpr BOOL similar(SelfCRef b) const { return min.similar(b.min) && max.similar(b.max); }
    ...
    constexpr SelfRef modify(T x, T y, T z)
    {
    ...
    constexpr SelfRef xform(SelfCRef B, const Tmatrix& m)
    {
    ...
    constexpr SelfRef xform(const Tmatrix& m)
    {
        Self b;
        b.set(*this);
        return xform(b, m);
    ...
    constexpr SelfCRef getcenter(Tvector& C) const
    {
    ...
    constexpr SelfCRef get_CD(Tvector& bc, Tvector& bd) const // center + dimensions
    {
    ...
    constexpr SelfRef scale(float s) // 0.1 means make 110%, -0.1 means make 90%
    {
        Fvector bd;
    ...
    constexpr SelfCRef getsphere(Tvector& C, T& R) const
    {
        getcenter(C);
        R = C.distance_to(max);
        return *this;
    ...
    constexpr ERP_Result Pick2(const Tvector& origin, const Tvector& dir, Tvector& coord)
    {
        BOOL Inside = TRUE;
        Tvector MaxT;
    ...
    constexpr void getpoint(int index, Tvector& result) const
    {
        switch (index)
        {
    ...
    constexpr SelfRef modify(SelfCRef src, const Tmatrix& M)
    {
        Tvector pt;
        for (int i = 0; i < 8; i++)
        {
