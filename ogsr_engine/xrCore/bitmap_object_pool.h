#ifndef xrBitmapObjectPoolH
#define xrBitmapObjectPoolH
...
class BitmapObjectPoolImpl
{
    ...
    pool_block* next_block(pool_block* block)
    {
        ...
        if (next == nullptr)
        {
            ...
            if (block->next.compare_exchange_strong(next, newBlock, std::memory_order_acq_rel, std::memory_order_acquire))
            {
                next = newBlock;
            }
            else
...
public:
    ...
    private:
        pool_block* block;
        uint64_t bit;
        ScopedPoolObject(T& Value, pool_block* Block, uint64_t Bit) : value{Value}, block{Block}, bit{Bit} {}
    ...
    ScopedPoolObject acquire_scoped()
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                static_cast<Derived*>(this)->initialize(static_cast<void*>(&block->get(bitIdx)));
    ...
    void for_each_available(Fn func)
    {
