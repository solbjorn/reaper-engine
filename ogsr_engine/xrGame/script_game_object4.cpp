...
u32 CScriptGameObject::play_hud_animation(LPCSTR anim, bool mix_in, u32 state, float speed)
{
    ...
    return k->PlayHUDMotion(anim, mix_in, state != std::numeric_limits<u32>::max() ? state : k->GetState(), speed);
}
...
void CScriptGameObject::addFeelTouch(f32 radius, sol::object lua_object, sol::function new_delete)
{
    addFeelTouch(radius, std::move(lua_object), std::move(new_delete), {});
}
