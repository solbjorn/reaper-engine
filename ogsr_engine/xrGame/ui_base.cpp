...
void ui_core::pp_start()
{
    ...
    g_current_font_scale.set(float(::Render->getTarget()->get_width(R__IMM_CTX_ID)) / float(Device.dwWidth),
                             float(::Render->getTarget()->get_height(R__IMM_CTX_ID)) / float(Device.dwHeight));
}
