...
tmc::task<bool> CWeapon::net_Spawn(CSE_Abstract* DC)
{
    ...
    {
        XR_LOG_ERROR("{}: wrong m_ammoType[{}/{}]", cName(), m_ammoType, std::ssize(m_ammoTypes) - 1);
        m_ammoType = 0;
    ...
    if (iAmmoElapsed)
    {
        ...
        {
            XR_LOG_ERROR("{}: wrong iAmmoElapsed[{}/{}]", cName(), iAmmoElapsed, iMagazineSize);
...
void CWeapon::SaveAttachableParams()
{
    ...
    XR_LOG_NOTICE("Data saved to [{}]", pHudCfg.fname());
}
...
void CWeapon::update_visual_bullet_textures(const bool forced)
{
    ...
    for (const auto& tex_name : bullet_textures_in_model)
    {
        ...
        if (textures.empty())
        {
            XR_LOG_ERROR("Can't find texture [{}] for [{}]", tex_name, cNameSect());
            continue;
        }
