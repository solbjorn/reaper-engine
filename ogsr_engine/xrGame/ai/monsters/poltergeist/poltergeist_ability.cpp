...
void CPoltergeist::PhysicalImpulse(const Fvector& position)
{
    ...
    const auto nobj = Level().ObjectSpace.GetNearest(m_nearest, position, IMPULSE_RADIUS, nullptr);
    if (nobj == 0)
        return;

    u32 index = Random.randI((u32)nobj);
    ...
    for (u32 i = 0; i < TRACE_ATTEMPT_COUNT; i++)
    {
        ...
        if (Level().ObjectSpace.RayPick(position, dir, TRACE_DISTANCE, collide::rqtStatic, l_rq, nullptr))
        {
            if (l_rq.range < TRACE_DISTANCE)
            {
                ...
                SGameMtlPair* mtl_pair = GMLib.GetMaterialPair(material().self_material_idx(), Level().ObjectSpace.GetStaticTris()[l_rq.element].material);
                if (!mtl_pair)
                    continue;
