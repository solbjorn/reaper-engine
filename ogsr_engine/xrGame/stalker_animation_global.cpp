...
MotionID CStalkerAnimationManager::global_critical_hit()
{
    ...
    VERIFY2(weapon,
            xr::format("current active item: {}",
                       object().inventory().ActiveItem() != nullptr ? std::string_view{object().inventory().ActiveItem()->object().cName()} :
                                                                      std::string_view{"no active item"}));
    ...
    return (global().select(
        m_data_storage->m_part_animations.A[MonsterSpace::eBodyStateStand].m_global.A[object().critical_wound_type() + 6 * (animation_slot - 1)].A,
