...
void STorsoWpn::Create(IKinematicsAnimated* K, LPCSTR base0, LPCSTR base1)
{
    ...
    if (std::is_eq(xr_strcmp(base0, "norm")) && std::is_eq(xr_strcmp(base1, "_0")))
        moving[eIdle] = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso_5_aim_1")});
    else
        moving[eIdle] = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_aim_1")});

    moving[eWalk] = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_aim_2")});
    moving[eRun] = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_aim_3")});
    moving[eSprint] = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_escape_0")});
    zoom = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_aim_0")});
    holster = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_holster_0")});
    draw = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_draw_0")});
    reload = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_reload_0")});
    reload_1 = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_reload_1")});
    reload_2 = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_reload_2")});
    drop = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_drop_0")});
    attack = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_attack_1")});
    attack_zoom = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_attack_0")});
    fire_idle = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_attack_1")});
    fire_end = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_torso", base1, "_attack_2")});
    all_attack_0 = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_all", base1, "_attack_0")});
    all_attack_1 = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_all", base1, "_attack_1")});
    all_attack_2 = K->ID_Cycle_Safe(shared_str{strconcat(sizeof(buf), buf, base0, "_all", base1, "_attack_2")});
}
void SAnimState::Create(IKinematicsAnimated* K, LPCSTR base0, LPCSTR base1)
{
    char buf[128];

    legs_fwd = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base0, base1, "_fwd_0")});
    legs_back = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base0, base1, "_back_0")});
    legs_ls = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base0, base1, "_ls_0")});
    legs_rs = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base0, base1, "_rs_0")});
}
...
void SActorState::CreateClimb(IKinematicsAnimated* K)
{
    ...
    // climb anims
    strcpy_s(base, "cl");
    legs_idle = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_idle_1")});
    m_torso_idle = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_torso_0_aim_0")});
    ...
    jump_begin = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_jump_begin")});
    jump_idle = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_jump_idle")});
    landing[0] = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_jump_end")});
...
void SActorState::Create(IKinematicsAnimated* K, LPCSTR base)
{
    ...
    m_torso_idle = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_torso_5_aim_0")});
    m_head_idle = K->ID_Cycle(shared_str{"head_idle_0"});
    jump_begin = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_jump_begin")});
    jump_idle = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_jump_idle")});
    landing[0] = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_jump_end")});
    landing[1] = K->ID_Cycle(shared_str{strconcat(sizeof(buf), buf, base, "_jump_end_1")});
...
{
    // leg anims
    legs_fwd = K->ID_Cycle(shared_str{"norm_escape_00"});
    legs_ls = K->ID_Cycle(shared_str{"norm_escape_ls_00"});
    legs_rs = K->ID_Cycle(shared_str{"norm_escape_rs_00"});
}

void SActorMotions::Create(IKinematicsAnimated* V)
{
    m_dead_stop = V->ID_Cycle(shared_str{"norm_dead_stop_0"});
...
void CActor::g_SetAnimation(u32 mstate_rl)
{
    ...
    if (!M_legs)
    {
        if ((mstate_rl & ACTOR_DEFS::mcCrouch) && !isActorAccelerated(mstate_rl, IsZoomAimingMode())) //!(mstate_rl&mcAccel))
            M_legs = smart_cast<IKinematicsAnimated*>(Visual())->ID_Cycle(shared_str{"cr_idle_1"});
        else
            M_legs = ST->legs_idle;
    }
