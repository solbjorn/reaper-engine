...
{
    sol::state_view(L).new_usertype<CUIDialogWndEx>(
        "CUIScriptWnd", sol::no_constructor, sol::call_constructor, sol::factories(std::make_unique<CUIDialogWndEx>), "pure",
        [](std::unique_ptr<CUIDialogWndEx>& self) -> std::unique_ptr<DLL_Pure> { return std::unique_ptr<DLL_Pure>(static_cast<DLL_Pure*>(self.release())); }, "AddCallback",
        sol::overload(sol::resolve<void(LPCSTR, s16, const luabind::functor<void>&)>(&CUIDialogWndEx::AddCallback),
                      sol::resolve<void(LPCSTR, s16, const luabind::functor<void>&, const luabind::object&)>(&CUIDialogWndEx::AddCallback)),
        ...
        "OnKeyboard", &CUIDialogWndEx::OnKeyboard, "Update", &CUIDialogWndEx::Update, "Dispatch", &CUIDialogWndEx::Dispatch, "Load", &CUIDialogWndEx::Load, sol::base_classes,
        xr_sol_bases<CUIDialogWndEx>());
}
