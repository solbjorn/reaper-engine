...
void SArtefactActivation::SpawnAnomaly()
{
    ...
    Fvector pos;
    m_af->Center(pos);
    const u32 lvid = m_af->UsedAI_Locations() ? m_af->ai_location().level_vertex_id() : ai().level_graph().nearest_vertex_id(pos);
    ...
    XR_LOG_DEBUG("Artefact [{}] spawned a zone [{}] at [{}]", m_af->cName(), zone_sect, Device.fTimeGlobal);
}
