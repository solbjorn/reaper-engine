...
namespace
{
constexpr float EFFECTOR_RADIUS{30.f};
constexpr u16 TEST_RAYS_PER_OBJECT{5};
constexpr u16 BLASTED_OBJ_PROCESSED_PER_FRAME{3};
constexpr float exp_dist_extinction_factor{3.f};
                                  ...
                                  const float expl_radius)
{
    ...
    for (u16 i = 0; i < TEST_RAYS_PER_OBJECT; ++i)
    {
        ...
        if (ph_dbg_draw_mask.test(phDbgDrawExplosions))
        {
            XR_LOG_TRACE_L1("Dist {},effect R {}", mag, expl_radius);
            XR_LOG_TRACE_L1("Test pass effect {}", add_eff);
            XR_LOG_TRACE_L1("S effect {}", _sqrt(l_S / max_s));
            XR_LOG_TRACE_L1("Dist/overlap effect, {}", add_eff / _sqrt(l_S / max_s));
        }
#else
        float l_S = effective_volume *
            (_abs(l_dir.dotproduct(obj_xform.i)) / l_d.x + _abs(l_dir.dotproduct(obj_xform.j)) / l_d.y + _abs(l_dir.dotproduct(obj_xform.k)) / l_d.z);
...
#ifdef DEBUG
    if (ph_dbg_draw_mask.test(phDbgDrawExplosions))
        XR_LOG_TRACE_L1("Damage effect {}", effect / TEST_RAYS_PER_OBJECT);
#endif
...
{
    if (!m_bDynamicParticles || !m_pExpParticle || !m_pExpParticle->IsPlaying())
        return;
