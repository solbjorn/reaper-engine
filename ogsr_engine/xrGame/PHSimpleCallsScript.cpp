...
{
    sol::state_view(L).new_usertype<CPHCallOnStepCondition>(
...
{
    sol::state_view(L).new_usertype<CPHExpireOnStepCondition>("phcondition_expireonstep", sol::no_constructor, sol::call_constructor,
                                                              sol::constructors<CPHExpireOnStepCondition()>(), sol::base_classes, xr_sol_bases<CPHExpireOnStepCondition>());
}

void CPHConstForceAction::script_register(lua_State* L)
{
    sol::state_view(L).new_usertype<CPHConstForceAction>("phaction_constforce", sol::no_constructor, sol::call_constructor,
