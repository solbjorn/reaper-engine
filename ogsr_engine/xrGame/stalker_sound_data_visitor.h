////////////////////////////////////////////////////////////////////////////
//	Module 		: stalker_sound_data_visitor.h
//	Created 	: 02.02.2005
//  Modified 	: 02.02.2005
//	Author		: Dmitriy Iassenev
//	Description : Stalker sound data visitor
////////////////////////////////////////////////////////////////////////////

#pragma once

#include "sound_user_data_visitor.h"

class CAI_Stalker;
class CStalkerSoundData;

class CStalkerSoundDataVisitor final : public CSound_UserDataVisitor
{
    RTTI_DECLARE_TYPEINFO(CStalkerSoundDataVisitor, CSound_UserDataVisitor);

private:
    CAI_Stalker* m_object;

public:
    inline explicit CStalkerSoundDataVisitor(CAI_Stalker* object);
    ~CStalkerSoundDataVisitor() override;

    void visit(CStalkerSoundData* data) override;
    [[nodiscard]] inline CAI_Stalker& object() const;
};

#include "stalker_sound_data_visitor_inline.h"
