...
void CAttachmentOwner::reload(LPCSTR section)
{
    ...
    m_attach_item_sections.reserve(item_count);
    for (u32 i = 0; i < item_count; ++i)
        m_attach_item_sections.emplace_back(_GetItem(attached_sections, i, current_item_section));
}
...
tmc::task<void> CAttachmentOwner::net_Destroy()
{
    ...
    if (!attached_objects().empty())
        XR_LOG_ERROR("Object {} has attached items", smart_cast<CGameObject*>(this)->cName());
#endif
...
{
    for (auto* item : m_attached_objects)
...
void CAttachmentOwner::attach(CInventoryItem* inventory_item)
{
    ...
    if (can_attach(inventory_item))
    {
        ...
        const auto bone_id = XR_ASSERT_VAL(smart_cast<IKinematics*>(game_object->Visual())->LL_BoneID(attachable_item->bone_name()) != BI_NONE,
                                           "attachment bone not found", game_object->cNameSect(), game_object->Visual()->getDebugName(),
                                           smart_cast<CGameObject*>(inventory_item)->cNameSect(), attachable_item->bone_name());
...
CAttachableItem* CAttachmentOwner::attachedItem(const shared_str& section) const
{
    for (auto item : m_attached_objects)
    {
        if (std::is_eq(xr_strcmp(item->item().object().cNameSect(), section)) && !item->item().IsInvalid())
            return item;
    }
