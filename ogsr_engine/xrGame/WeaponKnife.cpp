...
void CWeaponKnife::KnifeStrike(u32 state, const Fvector& pos, const Fvector& dir)
{
    ...
    if (apply)
    {
        ...
        SBullet& bullet = Level().BulletManager().AddBullet(pos, dir, m_fStartBulletSpeed, cur_fHit, cur_fHitImpulse, H_Parent()->ID(), ID(), cur_eHitType,
                                                            fireDistance, cartridge, send_hit);
        if (ParentIsActor())
