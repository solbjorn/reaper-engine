...
void CCar::DbgCreatePlots()
{
    ...
    //////////////////////////////
    int y_pos = 0, y_w = 100;
    m_dbg_power_rpm.Init(CallMe::fromMethod<&CCar::Parabola>(this), m_min_rpm, m_max_rpm, 0, y_pos, 500, y_w, 1000, D3DCOLOR_XRGB(0, 0, 255));
    ...
    m_dbg_torque_rpm.Init(CallMe::fromMethod<&CCar::TorqueRpmFun>(this), m_min_rpm, m_max_rpm, 0, y_pos, 500, y_w, 1000);
...
void CCar::DbgUbdateCl()
{
    if (m_pPhysicsShell && OwnerActor() && static_cast<CObject*>(Owner()) == Level().CurrentViewEntity())
    {
        if (ph_dbg_draw_mask.test(phDbgDrawCarDynamics))
        {
            ...
            HUD().Font().pFontStat->OutNext("speed, {} km/hour", v.magnitude() / 1000.0f * 3600.0f);
            HUD().Font().pFontStat->SetColor(D3DCOLOR_XRGB(255, !b_transmission_switching * 255, !b_transmission_switching * 255));
            HUD().Font().pFontStat->OutNext("Transmission num:      [{}]", m_current_transmission_num);
            HUD().Font().pFontStat->SetColor(color_rgba(0xff, 0xff, 0xff, 0xff));
