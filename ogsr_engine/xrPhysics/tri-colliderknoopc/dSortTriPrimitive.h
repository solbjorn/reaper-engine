...
[[nodiscard]] constexpr bool negative_tri_set_ignored_by_positive_tri(const Triangle& neg_tri, const Triangle& pos_tri, std::span<const Fvector4> V_array)
{
...
IC int dcTriListCollider::dSortTriPrimitiveCollide(dxGeom* o1, dxGeom* o2, int flags, dContactGeom* contact, int skip, const Fvector& AABB)
{
    ...
    for (I = B; I != E; ++I)
    {
        ...
        if (tri.dist < 0.f)
        {
            ...
            if (debug_output().ph_dbg_draw_mask().test(phDBgDrawNegativeTries))
                debug_output().DBG_DrawTri(Tr, V_array, D3DCOLOR_XRGB(0, 0, 255));
#endif
            ...
            if ((!(last_pos_dist < 0.f)) || b_pushing)
                if (__aabb_tri(Point(p), Point((const float*)&AABB), vertices))
                {
                    ...
                    SGameMtl* material = XR_ASSERT_VAL(GMLib.GetMaterialByIdx(Tr->material) != nullptr);
                    ...
                    if (!b_passable && (b_pased || (contain_pos && no_last_pos)))
                    {
                        ...
                        if (neg_depth > tri.depth && (!(*pushing_neg || spushing_neg) || dDOT(neg_tri.norm, tri.norm) > -M_SQRT1_2) &&
                            (!(*pushing_b_neg || spushing_b_neg) || dDOT(b_neg_tri.norm, tri.norm) > -M_SQRT1_2)) // exclude switching on opposite side
        ...
        else
        {
            ...
            if (debug_output().ph_dbg_draw_mask().test(phDBgDrawPositiveTries))
                debug_output().DBG_DrawTri(Tr, V_array, D3DCOLOR_XRGB(255, 0, 0));
#endif
    ...
    if (intersect)
    {
        if (neg_depth < dInfinity)
        {
            ...
            {
                XR_ASSERT(neg_tri.T != nullptr);
                XR_DEBUG_ASSERT(std::isfinite(neg_tri.dist));
    ...
    if (b_neg_depth < dInfinity)
    {
        ...
        if (no_last_pos)
        {
            for (auto& it : pos_tries)
            {
                XR_ASSERT(b_neg_tri.T != nullptr);
                XR_DEBUG_ASSERT(std::isfinite(b_neg_tri.dist));
