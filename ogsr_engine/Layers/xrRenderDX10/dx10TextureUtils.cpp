...
namespace
{
constexpr std::array<std::pair<D3DFORMAT, DXGI_FORMAT>, 17> TextureFormatList{{
    ...
    // D3DFMT_DXT2 DXGI_FORMAT_BC1_UNORM & DXGI_FORMAT_BC1_UNORM_SRGB Note: DXT1 and DXT2 are the same from an API/hardware perspective... only difference was
    // 'premultiplied alpha', which can be tracked by an application and doesn't need a separate format. D3DFMT_DXT3 DXGI_FORMAT_BC2_UNORM &
    // DXGI_FORMAT_BC2_UNORM_SRGB D3DFMT_DXT4 DXGI_FORMAT_BC2_UNORM & DXGI_FORMAT_BC2_UNORM_SRGB Note: DXT3 and DXT4 are the same from an API/hardware
    // perspective... only difference was 'premultiplied alpha', which can be tracked by an application and doesn't need a separate format. D3DFMT_DXT5
    // DXGI_FORMAT_BC3_UNORM & DXGI_FORMAT_BC3_UNORM_SRGB D3DFMT_D16 & D3DFMT_D16_LOCKABLE DXGI_FORMAT_D16_UNORM D3DFMT_D32 Not available D3DFMT_D15S1 Not
    // available D3DFMT_D24S8 Not available
    ...
    // D3DDECLTYPE_UBYTE4 DXGI_FORMAT_R8G8B8A8_UINT Note: Shader gets UINT values, but if Direct3D 9 style integral floats are needed (0.0f, 1.0f... 255.f),
    // UINT can just be converted to float32 in shader. D3DDECLTYPE_SHORT2 DXGI_FORMAT_R16G16_SINT Note: Shader gets SINT values, but if Direct3D 9 style
    // integral floats are needed, SINT can just be converted to float32 in shader. D3DDECLTYPE_SHORT4 DXGI_FORMAT_R16G16B16A16_SINT Note: Shader gets SINT
    // values, but if Direct3D 9 style integral floats are needed, SINT can just be converted to float32 in shader. D3DDECLTYPE_UBYTE4N
    // DXGI_FORMAT_R8G8B8A8_UNORM D3DDECLTYPE_SHORT2N DXGI_FORMAT_R16G16_SNORM D3DDECLTYPE_SHORT4N DXGI_FORMAT_R16G16B16A16_SNORM D3DDECLTYPE_USHORT2N
    // DXGI_FORMAT_R16G16_UNORM D3DDECLTYPE_USHORT4N DXGI_FORMAT_R16G16B16A16_UNORM D3DDECLTYPE_UDEC3 Not available D3DDECLTYPE_DEC3N Not available
...
{
    if (const auto it = std::ranges::find(TextureFormatList, dx9FMT, &std::pair<D3DFORMAT, DXGI_FORMAT>::first); it != TextureFormatList.end())
        return it->second;

    XR_PANIC("no corresponding texture format", dx9FMT);
}
} // namespace dx10TextureUtils
