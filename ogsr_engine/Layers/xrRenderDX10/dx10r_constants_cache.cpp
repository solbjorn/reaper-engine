...
void R_constants::flush(CBackend& cmd_list)
{
    const auto context_id = cmd_list.context_id;

    for (auto [vc, pc, gc, hc, dc, cc] : std::views::zip(cmd_list.m_aVertexConstants, cmd_list.m_aPixelConstants, cmd_list.m_aGeometryConstants,
                                                         cmd_list.m_aHullConstants, cmd_list.m_aDomainConstants, cmd_list.m_aComputeConstants))
    {
        if (vc)
            vc->Flush(context_id);
        if (pc)
