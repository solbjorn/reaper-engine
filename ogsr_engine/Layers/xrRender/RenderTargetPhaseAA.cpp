#include "stdafx.h"

void CRenderTarget::ProcessSMAA()
{
    XR_TRACY_ZONE_SCOPED();

    auto& cmd_list = RImplementation.get_imm_context().cmd_list;

    constexpr f32 d_Z{EPS_S};
    constexpr f32 d_W{1.0f};
    constexpr u32 C{color_rgba(0, 0, 0, 255)};

    const f32 _w = gsl::narrow_cast<f32>(Device.dwWidth);
    const f32 _h = gsl::narrow_cast<f32>(Device.dwHeight);

    // Half-pixel offset (DX9 only)
    constexpr Fvector2 p0{0.0f, 0.0f}, p1{1.0f, 1.0f};

    // Phase 0: edge detection ////////////////////////////////////////////////
    u_setrt(cmd_list, rt_smaa_edgetex, {}, {}, nullptr);
    cmd_list.set_CullMode(CULL_NONE);
    cmd_list.set_Stencil(TRUE, D3DCMP_ALWAYS, 0x1, 0, 0, D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE, D3DSTENCILOP_KEEP);

    for (u32 i = 0; i < 3; i++)
        cmd_list.ClearRT(cmd_list.get_RT(i), {});

    // Fill vertex buffer
    auto verts = cmd_list.Vertex.Lock<FVF::TL>(4);

    verts[0].set(EPS, _h + EPS, d_Z, d_W, C, p0.x, p1.y);
    verts[1].set(EPS, EPS, d_Z, d_W, C, p0.x, p0.y);
    verts[2].set(_w + EPS, _h + EPS, d_Z, d_W, C, p1.x, p1.y);
    verts[3].set(_w + EPS, EPS, d_Z, d_W, C, p1.x, p0.y);

    auto Offset = cmd_list.Vertex.Unlock<FVF::TL>(4);

    // Draw COLOR
    cmd_list.set_Element(s_pp_antialiasing->E[2]);
    cmd_list.set_Geometry(g_combine);
    cmd_list.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);

    // Phase 1: blend weights calculation ////////////////////////////////////
    u_setrt(cmd_list, rt_smaa_blendtex, {}, {}, nullptr);
    cmd_list.set_CullMode(CULL_NONE);
    cmd_list.set_Stencil(TRUE, D3DCMP_EQUAL, 0x1, 0, 0, D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE, D3DSTENCILOP_KEEP);

    for (u32 i = 0; i < 3; i++)
        cmd_list.ClearRT(cmd_list.get_RT(i), {});

    // Fill vertex buffer
    verts = cmd_list.Vertex.Lock<FVF::TL>(4);

    verts[0].set(EPS, _h + EPS, d_Z, d_W, C, p0.x, p1.y);
    verts[1].set(EPS, EPS, d_Z, d_W, C, p0.x, p0.y);
    verts[2].set(_w + EPS, _h + EPS, d_Z, d_W, C, p1.x, p1.y);
    verts[3].set(_w + EPS, EPS, d_Z, d_W, C, p1.x, p0.y);

    Offset = cmd_list.Vertex.Unlock<FVF::TL>(4);

    // Draw COLOR
    cmd_list.set_Element(s_pp_antialiasing->E[3]);
    cmd_list.set_Geometry(g_combine);
    cmd_list.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);

    // Phase 2: neighbour blend //////////////////////////////////////////////
    ref_rt& dest_rt = RImplementation.o.dx10_msaa ? rt_Generic : rt_Color;
    u_setrt(cmd_list, dest_rt, {}, {}, nullptr);

    cmd_list.set_CullMode(CULL_NONE);
    cmd_list.set_Stencil(FALSE);

    // Fill vertex buffer
    verts = cmd_list.Vertex.Lock<FVF::TL>(4);

    verts[0].set(EPS, _h + EPS, d_Z, d_W, C, p0.x, p1.y);
    verts[1].set(EPS, EPS, d_Z, d_W, C, p0.x, p0.y);
    verts[2].set(_w + EPS, _h + EPS, d_Z, d_W, C, p1.x, p1.y);
    verts[3].set(_w + EPS, EPS, d_Z, d_W, C, p1.x, p0.y);

    Offset = cmd_list.Vertex.Unlock<FVF::TL>(4);

    // Draw COLOR
    cmd_list.set_Element(s_pp_antialiasing->E[4]);
    cmd_list.set_Geometry(g_combine);
    cmd_list.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);

    cmd_list.context()->CopyResource(rt_Generic_0->pTexture->surface_get(), dest_rt->pTexture->surface_get());
}
