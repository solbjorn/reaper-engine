...
struct additional_bone_transform // Описывает постоянное смещение для указанной кости --#SM+#--
{
    ...
    void setRotGlobal(Fvector& vRotOfs) { setRotGlobal(vRotOfs.x, vRotOfs.y, vRotOfs.z); }
    ...
    void setRotLocal(Fvector& vRotOfs) { setRotLocal(vRotOfs.x, vRotOfs.y, vRotOfs.z); }

    // Установить смещение позиции кости
    void setPosOffset(const float _x, const float _y, const float _z) { m_transform.translate_over(_x, _y, _z); }

    void setPosOffset(Fvector& vPosOfs) { setPosOffset(vPosOfs.x, vPosOfs.y, vPosOfs.z); }
};
} // namespace KinematicsABT
