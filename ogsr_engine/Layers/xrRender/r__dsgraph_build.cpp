...
void R_dsgraph_structure::insert_dynamic(IRenderable* root, dxRender_Visual* pVisual, Fmatrix& xform, Fvector& Center)
{
    ...
    if (hud)
    {
        ...
        else
        {
            ...
            if (RImplementation.o.ssfx_core && !(sh->passes[0]->ps->dwFlags & xr_resource_flagged::RF_HUD_DISABLED))
...
[[nodiscard]] float GetDistFromCamera(const IRenderable* root, const Fvector& from_position)
{
...
[[nodiscard]] bool IsValuableToRender(const IRenderable* root, dxRender_Visual* pVisual, bool isStatic, bool sm, const Fmatrix* transform_matrix)
{
