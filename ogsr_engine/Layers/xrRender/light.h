#ifndef LAYERS_XRRENDER_LIGHT_H_INCLUDED
#define LAYERS_XRRENDER_LIGHT_H_INCLUDED

#include "../../xrcdb/ispatial.h"

#include "light_package.h"
#include "light_smapvis.h"

extern Fvector4 ps_ssfx_volumetric;

#define MIN_VIRTUAL_SIZE 0.01f

class light : public IRender_Light, public ISpatial
{
    RTTI_DECLARE_TYPEINFO(light, IRender_Light, ISpatial);

public:
    struct
    {
        u32 type : 4;
        u32 bStatic : 1;
        u32 bActive : 1;
        u32 bShadow : 1;
        u32 bVolumetric : 1;
        u32 bVolumetricManual : 1;
        u32 bHudMode : 1;
        u32 bMoveable : 1;
    } flags{};
    Fvector position;
    Fvector direction;
    Fvector right;
    float range;
    float cone;
    Fcolor color;

    vis_data hom;
    u32 frame_render;

    int omnipart_num;
    int sss_id;
    int sss_refresh;
    s8 sss_priority;
    light* omipart_parent;
    float distance;
    float distance_lpos;

    float m_volumetric_quality;
    float m_volumetric_intensity;
    float m_volumetric_distance;

    float virtual_size;

    float falloff; // precalc to make light equal to zero at light range
    float attenuation0; // Constant attenuation
    float attenuation1; // Linear attenuation
    float attenuation2; // Quadratic attenuation

    light* omnipart[6];

    std::array<smapvis, R__NUM_CONTEXTS> svis; // used for 6-cubemap faces

    ref_shader s_spot;
    ref_shader s_point;

    ref_shader s_spot_msaa;
    ref_shader s_point_msaa;

    u32 m_xform_frame;
    Fmatrix m_xform;

    struct _vis
    {
        u32 frame2test; // frame the test is sheduled to
        u32 query_id; // ID of occlusion query
        u32 query_order; // order of occlusion query
        bool visible; // visible/invisible
        bool pending; // test is still pending
        u16 smap_ID;
        float distance;
    } vis;

    union _xform
    {
        struct _D
        {
            Fmatrix combine[R__NUM_SUN_CASCADES];
            s32 minX, maxX;
            s32 minY, maxY;
        } D;
        struct _P
        {
            Fmatrix world;
            Fmatrix view;
            Fmatrix project;
            Fmatrix combine;
        } P;
        struct _S
        {
            Fmatrix view;
            Fmatrix project;
            Fmatrix combine;
            u32 size;
            u32 posX;
            u32 posY;
        } S;
    } X;

    void set_type(LT type) override { flags.type = type; }
    [[nodiscard]] u32 get_type() const override { return flags.type; }
    void set_active(bool b) override;
    [[nodiscard]] bool get_active() const override { return flags.bActive; }
    void set_shadow(bool b) override { flags.bShadow = b; }

    void set_volumetric(bool b, bool manual = false) override
    {
        flags.bVolumetricManual = manual;

        if (!manual && ps_ssfx_volumetric.x > 0)
            b = true;

        flags.bVolumetric = b;
    }

    void set_volumetric_quality(f32 fValue) override { m_volumetric_quality = fValue; }
    void set_volumetric_intensity(f32) override { m_volumetric_intensity = ps_ssfx_volumetric.y; }
    void set_volumetric_distance(f32) override { m_volumetric_distance = 1.0f; }

    void set_position(const Fvector3& P) override;
    void set_rotation(const Fvector3& D, const Fvector3& R) override;
    void set_cone(f32 angle) override;
    void set_range(f32 R) override;
    [[nodiscard]] f32 get_range() const override { return range; }
    void set_virtual_size(f32 S) override { virtual_size = (S > MIN_VIRTUAL_SIZE) ? S : MIN_VIRTUAL_SIZE; }

    void set_color(const Fcolor& C) override { color.set(C); }
    void set_color(f32 r, f32 g, f32 b) override { color.set(r, g, b, 1.0f); }
    [[nodiscard]] Fcolor get_color() const override { return color; }

    void set_texture(gsl::czstring name) override;
    void set_hud_mode(bool b) override { flags.bHudMode = b; }
    [[nodiscard]] bool get_hud_mode() const override { return flags.bHudMode; }

    void set_moveable(bool b) override { flags.bMoveable = b; }

    void spatial_move() override;
    [[nodiscard]] Fvector spatial_sector_point() const override;

    [[nodiscard]] IRender_Light* dcast_Light() override { return this; }

    vis_data& get_homdata();
    void xform_calc();
    void vis_prepare();
    void vis_update();
    void export_to(light_Package& dest);
    void set_attenuation_params(float a0, float a1, float a2, float fo);

    float get_LOD();

    light();
    ~light() override;
};

#endif // #define LAYERS_XRRENDER_LIGHT_H_INCLUDED
