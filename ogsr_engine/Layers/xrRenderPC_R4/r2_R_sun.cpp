...
tmc::task<void> CRender::sun_run()
{
    ...
    bool last_cascade_chain_mode;

    const bool need_to_render_sunshafts = Target->need_to_render_sunshafts();
    ...
    for (auto& cascade : m_sun_cascades)
        calculate_cascade(cascade);

    if (need_to_render_sunshafts)
        cascade->reset_chain = last_cascade_chain_mode;

    co_await tmc::spawn_many(m_sun_cascades | std::views::transform([this](auto& cascade) -> tmc::task<void> { return render_cascade(cascade); }));
}

void CRender::calculate_cascade(sun::cascade& cascade)
{
    XR_TRACY_ZONE_SCOPED();
    ...
    FixedConvexVolume<false> light_cuboid;
...
tmc::task<void> CRender::render_cascade(sun::cascade& cascade)
{
    XR_TRACY_ZONE_SCOPED();
    ...
    auto& sun = *smart_cast<light*>(Lights.sun._get());
    sun.X.D.combine[cascade_ind] = cascade.cull_xform;
    ...
    if (ps_r2_ls_flags.test(R2FLAG_SUN_DETAILS) && cascade_ind <= ps_ssfx_grass_shadows.x)
        co_await Details->Render(dsgraph.cmd_list, dm_fade * dm_fade * ps_ssfx_grass_shadows.y);
}

tmc::task<void> CRender::accumulate_cascade(u32 cascade_ind)
{
    XR_TRACY_ZONE_SCOPED();
...
tmc::task<void> CRender::sun_sync()
{
    ...
    ctx_id_t context_id = m_sun_cascades[SE_SUN_NEAR].context_id;
    get_context(context_id).cmd_list.submit();
    release_context(context_id);

    co_await std::move(middle);
    auto far = co_await tmc::fork_clang(accumulate_cascade(SE_SUN_FAR));

    context_id = m_sun_cascades[SE_SUN_MIDDLE].context_id;
    get_context(context_id).cmd_list.submit();
    release_context(context_id);

    co_await std::move(far);

    context_id = m_sun_cascades[SE_SUN_FAR].context_id;
    get_context(context_id).cmd_list.submit();
    release_context(context_id);
