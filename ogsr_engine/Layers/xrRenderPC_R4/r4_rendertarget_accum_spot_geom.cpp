...
void CRenderTarget::accum_spot_geom_create()
{
    ...
    u32 vSize = 3 * 4;
    XR_ASSERT(xr::hr(dx10BufferUtils::CreateVertexBuffer(&g_accum_spot_vb, du_cone_vertices.data(), vCount * vSize)));

    // Indices
    u32 iCount = DU_CONE_NUMFACES * 3;
    XR_ASSERT(xr::hr(dx10BufferUtils::CreateIndexBuffer(&g_accum_spot_ib, du_cone_faces.data(), iCount * 2)));
}
...
void CRenderTarget::accum_volumetric_geom_create()
{
    // vertices
    {
        ...
        XR_ASSERT(xr::hr(dx10BufferUtils::CreateVertexBuffer(&g_accum_volumetric_vb, &pSlice, vCount * vSize)));
    }
    ...
    // Indices
    {
        ...
        XR_ASSERT(xr::hr(dx10BufferUtils::CreateIndexBuffer(&g_accum_volumetric_ib, &Datap, iCount * 2)));
    }
}
