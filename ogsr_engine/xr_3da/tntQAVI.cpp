...
BOOL CAviPlayerCustom::Load(char* fname)
{
    ...
    AVIFileInit();
    PAVIFILE aviFile{};
    if (AVIFileOpenA(&aviFile, fname, OF_READ, nullptr) != AVIERR_OK)
        return FALSE;
    ...
    if (MMSYSERR_NOERROR != (res = mmioDescend(hmmioFile, &mmckinfoSubchunk, nullptr, MMIO_FINDLIST)) || mmckinfoSubchunk.cksize <= 4)
    {
        mmioClose(hmmioFile, 0);
        return FALSE;
    }
