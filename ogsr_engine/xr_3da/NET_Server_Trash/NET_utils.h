...
{
public:
    constexpr void construct(const void* data, u32 size)
    {
    ...
    // writing - main
    constexpr void write_start() { B.count = 0; }

    constexpr void w_begin(u16 type) // begin of packet 'type'
    {
        B.count = 0;
        w_u16(type);
    }
    ...
    constexpr void w_vec3(const Fvector& a)
    {
        *(Fvector*)(&B.data[B.count]) = a;
        w_advance(sizeof(Fvector));
    } // vec3

    constexpr void w_vec4(const Fvector4& a)
    {
        DirectX::XMStoreFloat4(reinterpret_cast<DirectX::XMFLOAT4*>(&B.data[B.count]), *reinterpret_cast<const DirectX::XMVECTOR*>(&a));
        w_advance(sizeof(Fvector4));
    } // vec4

    constexpr void w_u64(u64 a)
    {
        *(u64*)(&B.data[B.count]) = a;
        w_advance(sizeof(u64));
    } // qword (8b)

    constexpr void w_s64(s64 a)
    {
        *(s64*)(&B.data[B.count]) = a;
        w_advance(sizeof(s64));
    } // qword (8b)

    constexpr void w_u32(u32 a)
    {
        *(u32*)(&B.data[B.count]) = a;
        w_advance(sizeof(u32));
    } // dword (4b)

    constexpr void w_s32(s32 a)
    {
        *(s32*)(&B.data[B.count]) = a;
        w_advance(sizeof(s32));
    } // dword (4b)

    constexpr void w_u16(u16 a)
    {
        *(u16*)(&B.data[B.count]) = a;
        w_advance(sizeof(u16));
    } // word (2b)

    constexpr void w_s16(s16 a)
    {
        *(s16*)(&B.data[B.count]) = a;
        w_advance(sizeof(s16));
    } // word (2b)

    constexpr void w_u8(u8 a)
    {
        *(u8*)(&B.data[B.count]) = a;
        w_advance(sizeof(u8));
    } // byte (1b)

    constexpr void w_s8(s8 a)
    {
        *(s8*)(&B.data[B.count]) = a;
        w_advance(sizeof(s8));
    } // byte (1b)
    ...
    constexpr void w_angle16(float a) { w_float_q16(angle_normalize(a), 0, PI_MUL_2); }
    constexpr void w_angle8(float a) { w_float_q8(angle_normalize(a), 0, PI_MUL_2); }
    constexpr void w_dir(const Fvector& D) { w_u16(pvCompress(D)); }

    constexpr void w_sdir(const Fvector& D)
    {
        Fvector C;
    ...
    constexpr void w_stringZ(LPCSTR S) { w(S, (u32)xr_strlen(S) + 1); }
    ...
    constexpr void w_matrix(Fmatrix& M)
    {
        w_vec3(M.i);
        w_vec3(M.j);
        w_vec3(M.k);
    ...
    constexpr void w_clientID(ClientID& C) { w_u32(C.value()); }

    constexpr void w_chunk_open8(u32& position)
    {
        position = w_tell();
        w_u8(0);
    }
    ...
    // reading
    constexpr void read_start() { r_pos = 0; }

    [[nodiscard]] constexpr u32 r_begin(u16& type) // returns time of receiving
    {
        r_pos = 0;
        r_u16(type);
        return timeReceive;
    ...
    constexpr void r_vec4(Fvector4& A)
    {
        *reinterpret_cast<DirectX::XMVECTOR*>(&A) = DirectX::XMLoadFloat4(reinterpret_cast<const DirectX::XMFLOAT4*>(&B.data[r_pos]));
        r_advance(sizeof(Fvector4));
    } // vec4

    constexpr void r_float(float& A)
    {
        A = *(float*)(&B.data[r_pos]);
        r_advance(sizeof(float));
    } // float

    constexpr void r_u64(u64& A)
    {
        A = *(u64*)(&B.data[r_pos]);
        r_advance(sizeof(u64));
    } // qword (8b)

    constexpr void r_s64(s64& A)
    {
        A = *(s64*)(&B.data[r_pos]);
        r_advance(sizeof(s64));
    } // qword (8b)

    constexpr void r_u32(u32& A)
    {
        A = *(u32*)(&B.data[r_pos]);
        r_advance(sizeof(u32));
    } // dword (4b)

    constexpr void r_s32(s32& A)
    {
        A = *(s32*)(&B.data[r_pos]);
        r_advance(sizeof(s32));
    } // dword (4b)

    constexpr void r_u16(u16& A)
    {
        A = *(u16*)(&B.data[r_pos]);
        r_advance(sizeof(u16));
    } // word (2b)

    constexpr void r_s16(s16& A)
    {
        A = *(s16*)(&B.data[r_pos]);
        r_advance(sizeof(s16));
    } // word (2b)

    constexpr void r_u8(u8& A)
    {
        A = *(u8*)(&B.data[r_pos]);
        r_advance(sizeof(u8));
    } // byte (1b)

    constexpr void r_s8(s8& A)
    {
        A = *(s8*)(&B.data[r_pos]);
        r_advance(sizeof(s8));
    } // byte (1b)
    ...
    [[nodiscard]] constexpr Fvector r_vec3()
    {
        Fvector tmp;
    ...
    [[nodiscard]] constexpr Fvector4 r_vec4()
    {
        const auto tmp = DirectX::XMLoadFloat4(reinterpret_cast<const DirectX::XMFLOAT4*>(&B.data[r_pos]));
        r_advance(sizeof(Fvector4));
        return *reinterpret_cast<const Fvector4*>(&tmp);
    } // vec4

    [[nodiscard]] constexpr float r_float_q8(float min, float max)
    {
        float A;
    ...
    [[nodiscard]] constexpr float r_float_q16(float min, float max)
    {
        float A;
    ...
    [[nodiscard]] constexpr float r_float()
    {
        float tmp;
    ...
    constexpr void r_float_q16(float& A, float min, float max)
    {
    ...
    constexpr void r_angle8(float& A) { r_float_q8(A, 0, PI_MUL_2); }

    constexpr void r_dir(Fvector& A)
    {
    ...
    constexpr void r_sdir(Fvector& A)
    {
        u16& t = *(u16*)(&B.data[r_pos]);
        r_advance(sizeof(u16));
    ...
    constexpr void r_stringZ(LPSTR S)
    {
    ...
    constexpr void r_stringZ(xr_string& dest)
    {
        dest = LPCSTR(&B.data[r_pos]);
        r_advance(u32(dest.size() + 1));
    }

    constexpr void r_stringZ(shared_str& dest)
    {
        dest._set(LPCSTR(&B.data[r_pos]));
        r_advance(dest.size() + 1);
    }

    constexpr void r_matrix(Fmatrix& M)
    {
        r_vec3(M.i);
        M.vm[0].set(M.i, 0);
        r_vec3(M.j);
    ...
    constexpr void r_clientID(ClientID& C)
    {
        u32 tmp;
        r_u32(tmp);
        C.set(tmp);
