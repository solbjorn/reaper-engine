...
{
    lua.new_usertype<CSE_ALifeTrader>("cse_alife_trader", sol::no_constructor, sol::call_constructor, sol::factories(std::make_unique<CSE_ALifeTrader, LPCSTR>),
                                      "factory", &xr::server_factory<CSE_ALifeTrader>, "alive", &CSE_AlifeTrader_alive, "smart_terrain_id",
                                      &CSE_AlifeTrader__smart_terrain_id, sol::base_classes, xr::sol_bases<CSE_ALifeTrader>());
}

void CSE_ALifeCustomZone::script_register(sol::state_view& lua)
{
    lua.new_usertype<CSE_ALifeCustomZone>("cse_custom_zone", sol::no_constructor, sol::call_constructor,
                                          sol::factories(std::make_unique<CSE_ALifeCustomZone, LPCSTR>), sol::base_classes,
                                          xr::sol_bases<CSE_ALifeCustomZone>());
}

void CSE_ALifeAnomalousZone::script_register(sol::state_view& lua)
{
    lua.new_usertype<CSE_ALifeAnomalousZone>("cse_anomalous_zone", sol::no_constructor, sol::call_constructor,
                                             sol::factories(std::make_unique<CSE_ALifeAnomalousZone, LPCSTR>), "factory",
                                             &xr::server_factory<CSE_ALifeAnomalousZone>, "spawn_artefacts", &CSE_ALifeAnomalousZone::spawn_artefacts,
                                             sol::base_classes, xr::sol_bases<CSE_ALifeAnomalousZone>());
}
