...
#define CGameVertexTypePathManager \
    CPathManager<CGameGraph, _DataStorage, SGameLevel<_dist_type, _index_type, _iteration_type>, _dist_type, _index_type, _iteration_type>

TEMPLATE_SPECIALIZATION
IC void CGameVertexTypePathManager::setup(const _Graph* _graph, _DataStorage* _data_storage, xr_vector<_index_type>* _path,
...
{
    if (this->graph->vertex(this->data_storage->get_best().index())->level_id() == XR_ASSERT_VAL(m_evaluator != nullptr)->m_level_id)
    {
