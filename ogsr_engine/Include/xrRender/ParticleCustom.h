#ifndef ParticleCustom_included
#define ParticleCustom_included

class XR_NOVTABLE IParticleCustom : public virtual RTTI::Enable
{
    RTTI_DECLARE_TYPEINFO(IParticleCustom);

public:
    ~IParticleCustom() override = 0;

    virtual void OnDeviceCreate() = 0;
    virtual void OnDeviceDestroy() = 0;

    virtual void UpdateParent(const Fmatrix& m, const Fvector3& velocity, BOOL bXFORM) = 0;
    virtual void OnFrame(u32 dt) = 0;

    virtual void Play() = 0;
    virtual void Stop(BOOL bDefferedStop = TRUE) = 0;
    [[nodiscard]] virtual BOOL IsPlaying() = 0;

    [[nodiscard]] virtual u32 ParticlesCount() = 0;

    [[nodiscard]] virtual f32 GetTimeLimit() = 0;
    [[nodiscard]] virtual BOOL IsLooped() { return GetTimeLimit() < 0.f; }

    [[nodiscard]] virtual const shared_str Name() = 0;
    virtual void SetHudMode(BOOL b) = 0;
    [[nodiscard]] virtual BOOL GetHudMode() = 0;
};

inline IParticleCustom::~IParticleCustom() = default;

#endif //	ParticleCustom_included
